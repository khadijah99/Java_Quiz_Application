...
public class AddQuestionFrame extends JFrame {
	...
	JButton qimage, op1image, op2image, op3image, op4image, done, addAnotherQuestion, b3;
	JPanel p1, p2, p3, p4, p5, p6, p7;
	String qImage, op1, op2, op3, op4, s = "1";
	String ops[] = new String[4];
	ArrayList<Question> questions = new ArrayList<Question>();
	
	
	public AddQuestionFrame(Test t, Admin admin) {
        ...
        qimage.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				JFileChooser j = new JFileChooser("E://Project");
        ...
        Integer opt[] = {1,2,3,4};
        JComboBox<Integer> option = new JComboBox<Integer>(opt);
        ...
        option.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				s = option.getSelectedItem().toString();
					...
					JOptionPane.showMessageDialog(null, "Select correct option please", "Error Message", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
        ...
        addAnotherQuestion = new JButton("Add Another Question"); addAnotherQuestion.setFont(new Font("Times New Roman", Font.PLAIN, 20));
        
        p7  = new JPanel(new FlowLayout(0, 17, 0));
        p7.setBounds(270, 600, 500, 500);
        p7.setOpaque(false);
        p7.add(done); p7.add(addAnotherQuestion); 
        background.add(p7);
        

        
        addAnotherQuestion.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				else if(image1.isSelected()==true && (op1image.getText().equals("Add Image") || op2image.getText().equals("Add Image") || op3image.getText().equals("Add Image") || op4image.getText().equals("Add Image") ))
					JOptionPane.showMessageDialog(null, "Select image for all options", "Error Message", JOptionPane.ERROR_MESSAGE);
				...
				new AddQuestionFrame(t,admin);
				AddQuestionFrame.this.dispose();
		...
		b3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(text1.isSelected()) {
					...
					ops[0] = t2.getText(); ops[1] = op2; ops[2] = op3; ops[3] = op4;
				}
				...
				for (int x = 0 ; x < t.getQuestions().size(); x++) {
					tmarks += t.getQuestions().get(x).getMarks();
					ttime += t.getQuestions().get(x).timeAllocated();
					tquestion++;
				}
				int hours = ttime/3600;
				int mins = (ttime%3600)/60;
				int secs = (ttime%3600)%60;
				
				String time = hours+":"+mins+":"+secs;
				
				t.setTotalMarks(tmarks);
				t.setTotalTime(time);
				t.setTestName(t.getTestName());
				
				CreateTestFrame tnew = new CreateTestFrame(t,admin);
				
				tnew.j1.setText(t.getTestName());
				tnew.j2.setText(time);
				tnew.j3.setText(Integer.toString(t.getQuestions().size()-1));
				tnew.j4.setText(Integer.toString(t.getTotalMarks()));
				tnew.b1.setEnabled(true);
